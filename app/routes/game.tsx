...
export default function Game() {
  ...
  const checkShuffledCountries = () => {
    ...
    if (decodeCountries.length === 0) {
      // Fallback to region-based countries
      const region = localStorage.getItem(regionKey) as Region || "eu";
      return shuffleArray([...getCountriesForRegion(region)]);
    }
    return shuffleArray([...decodeCountries]);
  };

  const [shuffledCountries, setShuffledCountries] = useState<typeof euCountries>(
    []
  );
  ...
  const [incorectAnswers, setIncorrectAnswers] = useState<typeof euCountries>([]);
    ...
    <div>
      <div className="flex justify-center items-center flex-col gap-2 text-9xl m-4">
        <div className="text-4xl font-bold">
          {currentIndex + 1} de {shuffledCountries.length}
        </div>
      ...
      <div className="flex justify-center gap-3 mb-6">
        ...
        <div>
          <label htmlFor="capital-select" className="sr-only">
            Seleccionar capital
          </label>
          <select
            id="capital-select"
